...
export default function UsersPage() {
  ...
  const handlePageChange = async (newPage: number) => {
    setIsLoading(true);
    const response = await fetch(`/api/users?page=${newPage}&limit=${USERS_PER_PAGE}`);
    const data = await response.json();
    setUserList(data.users);
    setPage(newPage);
    setIsLoading(false);
  ...
  return (
    <div className="mt-5 ml-12 w-[80%]">
      ...
      {isDialogOpen && selectedUser && (
        <EditUserDialog user={selectedUser} onDelete={handleDelete} onClose={handleDialogClose} onSave={handleSave} setUser={setSelectedUser} />
      )}
    </div>
  );
}
...
function UserTable({ users, totalPages, page, onPageChange, onEdit }: UserTableProps) {
  return (
    <>
      <table className="w-full border">
        ...
        <tbody>
          {users && users.map((user) => (
            <tr key={user.id} onClick={() => onEdit(user)} className="cursor-pointer hover:opacity-80 transition-colors">
      ...
      <div className="mt-4 flex justify-center gap-2">
        ...
        <span className="py-2 px-4 border bg-muted text-white">{page}</span>
        <Button
          onClick={() => onPageChange(page + 1)}
