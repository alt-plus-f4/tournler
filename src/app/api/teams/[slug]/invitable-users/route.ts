 ...
 * Only that team's captain can invite, so only they can search here.
 */
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
	try {
		...
		const team = await db.cs2Team.findUnique({ where: { id: teamId }, select: { capitanId: true } });
		...
		// starting view) — just capped, not the entire user table.
		const where = {
			AND: [
				{ OR: [{ cs2TeamId: null }, { cs2TeamId: { not: teamId } }] },
		...
		const users = await db.user.findMany({
			where,
			select: { id: true, name: true, email: true, image: true, cs2TeamId: true },
			take: RESULT_LIMIT,
			orderBy: { name: 'asc' },
		});
