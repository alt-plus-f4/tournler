...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
	...
	if (teamId) {
		const teamInTournament = tournament.teams.some((team) => team.id === parseInt(teamId, 10));
...
export async function DELETE(request: Request, { params }: { params: Promise<{ slug: string }> }) {
	try {
		...
		return NextResponse.json({ message: 'Team removed from tournament successfully' }, { status: 200 });
	} catch (error) {
