...
import { getSideLabels, getWinningSide, type Match, type Side } from './types';

function formatLabel(match: Match) {
	const bo = `BO${match.bestOf ?? 1}`;
	if (!match.isPickup) return bo;
	return `${bo} · Pickup${match.pickupMode === 'CAPTAIN_DRAFT' ? ' · Captain draft' : ''}`;
}
...
export function RoomHeader({ match, quickBar, tabs }: { match: Match; quickBar?: ReactNode; tabs: ReactNode }) {
			...
			<div className={cn('flex min-w-0 flex-col items-center gap-3 sm:gap-5', isA ? 'sm:flex-row-reverse sm:justify-start' : 'sm:flex-row sm:justify-start', result === 'loss' && 'opacity-60')}>
				...
				<div className={cn('flex min-w-0 max-w-full flex-col items-center', isA ? 'sm:items-end' : 'sm:items-start')}>
					...
					<div className='mt-1.5 h-5'>
						{result === 'win' && <span className='rounded-sm bg-white px-1.5 py-0.5 text-[11px] font-black uppercase tracking-[0.12em] text-black'>Winner</span>}
	...
	return (
		<section className='relative isolate overflow-hidden border-b border-border bg-black'>
			...
			{(match.status === 'LIVE' || match.status === 'PAUSED') && <div className={cn('absolute inset-x-0 top-0 h-[3px]', match.status === 'LIVE' ? 'bg-red-500' : 'bg-yellow-400')} aria-hidden />}
					...
					<div className='flex min-w-0 flex-wrap items-center gap-x-5 gap-y-2'>
						<nav aria-label='Breadcrumb' className='flex min-w-0 items-center gap-1.5 text-sm text-muted-foreground'>
							{match.isPickup ? (
								...
								<Link href={`/tournaments/${match.tournament.id}`} className='truncate hover:text-white'>
									{match.tournament.name}
								</Link>
							)}
							<ChevronRight className='h-3.5 w-3.5 shrink-0' aria-hidden />
							<span className='shrink-0 text-white'>Match {match.id}</span>
							<span className='hidden shrink-0 sm:inline' aria-hidden>
								·
							</span>
							<span className='hidden shrink-0 font-mono text-xs sm:inline'>{formatLabel(match)}</span>
						</nav>
				...
				<div className='grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3 py-8 sm:gap-10 sm:py-12'>
					...
					<div className='flex flex-col items-center text-center'>
						{match.status === 'SCHEDULED' ? (
							<>
								<span className='text-3xl font-black text-neutral-600 sm:text-5xl'>VS</span>
							...
							<>
								<span className='font-mono text-5xl font-bold leading-none text-neutral-600 sm:text-7xl' aria-hidden>
							...
							<>
								<div className='flex items-baseline font-mono text-5xl font-bold tabular-nums leading-none sm:text-7xl' aria-label={`Score ${headlineA} to ${headlineB}`}>
									<span className={cn(winningSide === 'TEAM_B' ? 'text-neutral-500' : 'text-white')}>{headlineA}</span>
									<span className='mx-2 text-neutral-700 sm:mx-3'>:</span>
									<span className={cn(winningSide === 'TEAM_A' ? 'text-neutral-500' : 'text-white')}>{headlineB}</span>
								</div>
							</>
						)}
						{displayMap && (
