 ...
 */
export function TrophySlider({ trophies, className }: { trophies: TrophyAward[]; className?: string }) {
	const scrollerRef = useRef<HTMLUListElement>(null);
	const [canPrev, setCanPrev] = useState(false);
	const [canNext, setCanNext] = useState(false);

	const sorted = [...trophies].sort((a, b) => new Date(b.awardedAt).getTime() - new Date(a.awardedAt).getTime());
	...
	return (
		<section aria-labelledby='profile-trophies-label' className={className}>
			...
			<TooltipProvider delayDuration={150}>
				...
				>
					{sorted.map(({ badge, awardedAt }) => (
						<li key={badge.id} className='w-24 shrink-0 snap-start'>
							<Tooltip>
								<TooltipTrigger asChild>
									<div
										tabIndex={0}
										className='group flex flex-col items-center gap-2 rounded-md px-1 py-2 text-center outline-none transition-colors hover:bg-white/[0.04] focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring'
									>
								...
								<TooltipContent side='bottom' className='max-w-[240px] border-border bg-neutral-950 text-white'>
