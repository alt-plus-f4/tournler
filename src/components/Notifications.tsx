...
export default function Notifications({ userId }: NotificationsProps) {
	...
	if (!mounted) {
		return (
			<Button variant='ghost' size='icon' className='p-2 relative' aria-label='Notifications'>
				<Bell width={16} />
			</Button>
		);
	}
		...
		<Popover>
			<PopoverTrigger asChild>
				<Button variant='ghost' size='icon' className='p-2 relative'>
					<Bell width={16} />
					{hasNewNotifications && <div className='absolute text-xs -top-0 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-600'>{notifications?.length}</div>}
				</Button>
			</PopoverTrigger>
					...
					<div className='space-y-4 p-4'>
						...
						{hasNewNotifications &&
							notifications?.map(({ _id, text, _creationTime, type, teamId }) => (
								<div key={_id} className='grid grid-cols-[1fr_auto] items-start gap-4'>
									...
									<div>
										{type === 1 ? (
											<>
												<Button variant={'default'} onClick={() => handleAcceptInvite(_id, teamId || -1)}>
													<CheckIcon className='h-5 w-5' />
												</Button>
												<Button className='ml-2' variant={'outline'} onClick={() => handleDenyInvite(_id, teamId || -1)}>
													<X className='h-5 w-5' />
												</Button>
											</>
										) : (
											<Button className='ml-2' variant={'outline'} onClick={() => markAsRead({ id: _id })}>
												<X className='h-5 w-5' />
											</Button>
										)}
									</div>
